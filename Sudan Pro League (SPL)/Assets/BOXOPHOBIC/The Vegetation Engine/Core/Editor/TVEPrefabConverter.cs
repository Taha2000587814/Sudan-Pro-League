...
namespace TheVegetationEngine
{
    ...
    public class TVEPrefabConverter : EditorWindow
    {
        ...
        bool collectCommonMaterials = true;
        ...
        void OnEnable()
        {
            ...
            collectCommonMaterials = Convert.ToBoolean(SettingsUtils.LoadSettingsData(userFolder + "Converter Materials.asset", "True"));
        ...
        void DrawConversionSettings()
        {
            ...
            if (presetIndex != 0)
            {
                ...
                if (showAdvancedSettings)
                {
                    ...
                    {
                        GUILayout.BeginHorizontal();
                        GUILayout.Label(new GUIContent("Share Common Materials", "Share all common materials across multiple prefabs to the Assets Data/Shared Data folder."), GUILayout.Width(GUI_HALF_EDITOR_WIDTH - 5));
                        collectCommonMaterials = EditorGUILayout.Toggle(collectCommonMaterials);
                        GUILayout.EndHorizontal();
                        ...
                        if (collectCommonMaterials)
                        {
                            GUILayout.BeginHorizontal();
                            GUILayout.Label(new GUIContent("Show Material Sharing Dialogue", "Shows a dialogue box for choosing if the common materials should be shared across multiple assets."), GUILayout.Width(GUI_HALF_EDITOR_WIDTH - 5));
                ...
                if (collectConvertedData)
                {
                    ...
                    if (projectDataFolder != "")
                    {
                        ...
                        if (collectCommonMaterials)
                        {
                            ...
                            {
                                shareCommonMaterials = collectCommonMaterials;
                            }
                        }
                    }
                    else
        ...
        void StyledPrefab(TVEPrefabData prefabData)
        {
            ...
            if (prefabData.status == PrefabStatus.Unsupported || prefabData.status == PrefabStatus.ConvertedMissingBackup)
            {
        ...
        void SaveCollectSettings()
        {
            ...
            SettingsUtils.SaveSettingsData(userFolder + "Converter Materials.asset", collectCommonMaterials.ToString());
        ...
        void GetMeshConversionFromPreset()
        {
            ...
            {
                var useLine = GetOptionConditionFromLine(presetLines[i]);
        ...
        void GetMaterialConversionFromPreset(Material originalMaterial, Material instanceMaterial)
        {
            ...
            for (int i = 0; i < presetLines.Count; i++)
            {
                var useLine = GetMaterialConditionFromLine(presetLines[i], material);
        ...
        void GetDescriptionFromPreset()
        {
            ...
            if (presetAutoDetected)
            {
                ...
                //if (EditorGUIUtility.isProSkin)
                //{
                //    infoTitle = infoTitle + " <color=#f6d161>(Auto Detected)</color>";
                //}
                //else
                //{
                //    infoTitle = infoTitle + " <size=10><b><color=#e16f00>(Auto-Detected)</color></b></size>";
                //}
            }
        }
        ...
        void GetOutputsFromPreset()
        {
            ...
            for (int i = 0; i < presetLines.Count; i++)
            {
                ...
                if (outputPipeline != "")
                {
                    ...
                    {
                        outputValid = false;
                        infoError = "The current asset can only be converted in the " + outputPipeline + " render pipeline and then exported to you project using the Convert and Collect option!";
                    }
                }
            }
        }
        ...
        bool GetOptionConditionFromLine(string line)
        {
            var valid = true;
        ...
        bool GetMaterialConditionFromLine(string line, Material material)
        {
            var valid = true;
